...
export default function DetalleMedicion() {
    ...
    const estado = medicionAEditar && !doble ? calcularEstado(medicionAEditar.valor, tipo) : null
                ...
                >
                    ...
                    <View className="rounded-card bg-surface-raised border border-line-strong p-5 gap-4">
                        ...
                        <View className="flex-row items-center gap-1.5">
                            <Text className="text-[60px] font-extrabold text-content">
                                {medicionAEditar?.valor}{doble && ` / ${medicionAEditar?.valor_secundario}`}
                            </Text>
                            <Text className="text-body text-content-muted mb-1">{tipo?.unidad}</Text>
                        </View>
                        ...
                        {estado && (
                            <View className="flex-row items-center justify-between rounded-control bg-surface-raised border border-line px-4 py-3">
                                <View className="flex-row items-center gap-2">
                                    <View className={`w-3.5 h-3.5 rounded-[3px] ${ESTILO_ESTADO[estado].icono}`} />
                                    <Text className={`text-label font-bold ${ESTILO_ESTADO[estado].texto}`}>
                    ...
                    <View className="rounded-card bg-surface-raised border border-line-strong overflow-hidden">
                        <View className="flex-row items-center justify-between px-5 py-4 border-b border-line">
                            ...
                            <Text className="text-body font-semibold text-content">{formatearFecha(date, true, true)}</Text>
                        </View>
                        ...
                        <View className="flex-row items-center justify-between px-5 py-4 border-b border-line-strong">
                            ...
                            <Text className="text-body font-semibold text-content">{formatearHora(date.toLocaleTimeString())}</Text>
                        </View>

                        <View className="flex-row items-center justify-between px-5 py-4 border-b border-line-strong">
                            <Text className="text-body text-content-muted">Contexto</Text>
                            <Text className="text-body font-semibold text-content">{labelContexto(medicionAEditar?.contexto ?? null)}</Text>
                        </View>
                    ...
                    <View className="flex-row gap-3">
                        <Pressable className="flex-1 py-4 bg-surface-raised rounded-control border border-line-strong items-center active:bg-surface-sunken">
                            <Text className="text-body font-semibold text-content">Editar</Text>
                        </Pressable>

                        <Pressable className="flex-1 py-4 bg-surface-raised rounded-control border border-line-strong items-center active:bg-danger-subtle">
                            <Text className="text-body font-semibold text-danger">Eliminar</Text>
                        </Pressable>
                    </View>
