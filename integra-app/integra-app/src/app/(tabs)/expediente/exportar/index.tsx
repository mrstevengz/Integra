...
export default function ExportarScreen() {
    ...
    const textoQR = armarQREmergencia(perfil, alergias, contactos, condiciones, medicamentos)
    const largoDeMas = textoQR.length > LARGO

    const bloques = textoQR.split('\n\n')

    async function exportarPDF() {
        Alert.alert('Exportar', 'Esta funcionalidad no esta disponible en este momento')
    }
    ...
    return (
        <View className="flex-1">
            ...
            <TopBarSecondary active="Emergencia" tab1="Emergencia" tab2="Expediente" route1="/expediente/emergencia" route2="/cita/historial"/>

            <ScrollView
                className="flex-1 bg-surface"
                contentContainerStyle={{ padding: 20, paddingBottom: 100, paddingTop: 10 }}
            >
                {!generado ? (
                    <>
                        <View className = "flex-col border border-line-focus rounded-card p-5 mb-5 gap-2 bg-surface-sunken">
                            <Text className = "text-subheading font-semibold">Acceso de emergencia</Text>
                            <Text className="text-body text-content-muted mb-4">
                            Este codigo muestra informacion critica de salud para personal medico en caso de emergencia. No requiere que el paciente este consciente.
                            </Text>
                        </View>
                    ...
                    <>
                        {largoDeMas && (
                            <View className="bg-warning-subtle border-l-4 border-warning rounded-chip p-4 mb-5">
                                <Text className="text-caption text-warning-on-subtle">
                                    El codigo quedo largo ({textoQR.length} caracteres).
                                    Funciona, pero puede costar enfocarlo. Acerque bien la camara.
                                </Text>
                            </View>
                        )}
                        ...
                        <View className="bg-surface-raised rounded-card border border-line p-6 items-center">
                            ...
                            <Text className="text-caption text-content-subtle mt-5 text-center">
                                Apunte cualquier camara de telefono al codigo.
