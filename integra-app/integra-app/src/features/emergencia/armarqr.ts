...
function filtrarPorCreacion<T extends { created_at?: string | null }>(a: T, b: T): number {
...
export function armarQREmergencia(
    ...
    const edad = perfil.fecha_nacimiento ? `${getAge(perfil.fecha_nacimiento)} años` : null
    const nacimiento = perfil.fecha_nacimiento ? `- Fecha de nacimiento: ${new Date(perfil.fecha_nacimiento).toLocaleDateString()}` : ''
    ...
    const cabecera = [
        'INTEGRA - EMERGENCIA',
        nombre,
        informacion].filter(Boolean).join('\n')

    //Alergias

    const alergiasLista = alergias.sort(filtrarPorCreacion).map((a) => (a.severidad ? `${a.nombre} (${a.severidad})` : a.nombre))

    const contactosLista = contactos.sort(filtrarPorCreacion).slice(0, 3).map((c) => (c.telefono ? `${c.nombre} (${c.telefono})` : c.nombre))

    const condicionesLista = condiciones.sort(filtrarPorCreacion).slice(0, TOPE_CONDICIONES).map((c) => c.nombre)

    const medicamentosLista = medicamentos.sort(filtrarPorCreacion).filter((m) => m.activo).slice(0, TOPE_MEDICAMENTOS).map((m) => `${m.nombre} ${m.dosis}${m.unidad}`.trim())
